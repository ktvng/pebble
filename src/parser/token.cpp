...
TokenList LexLine(const std::string &line)
{
    ...
    int tokenNumber = 0;
    std::string removed = line;
    RemoveComment(line);
    // RemoveCommentBetter(&removed.at(0), &line.at(0));
    std::cout << line;
...
// TODO: (new idea) use left of token to get everything to the left of the #, discard everything else
// TokenList ParseComment(const std::string &line)
// {
//     TokenList tokens;
//     // what's the purpose of having both linePosition and tokenNumber in the previous method?
//     // line position never gets updated?
//     // size_t linePosition = 0;
//     // int tokenNumber = 0;
//     // std::string updatedLine = RemoveComment(line);
//     return LexLine(line);
// }
